...
export const multimeterSlice = createSlice({
  ...
  reducers: {
    ...
    setMeasurementResult: (state, action: PayloadAction<SetMeasurementPayload>) => {
      if (!SUPPORTED_MODES.includes(state.currentMode) && state.currentMode !== 'OFF') {
        state.displayValue = '---'; 
        state.measurementResult = null;
        state.measurementUnit = null;
      ...
      setProbeConnection: (state, action: PayloadAction<SetProbeConnectionPayload>) => {
        ...
        state.measurementUnit = null;
         if (state.currentMode === 'OFF') state.displayValue = '';
         else if (state.currentMode === 'ACV_750') state.displayValue = '0.00';
         else if (state.currentMode === 'OHM_200') state.displayValue = 'OL';
      },
  
      setProbePosition: (state, action: PayloadAction<SetProbePositionPayload>) => {
          state.probePositions[action.payload.probeColor] = action.payload.position;
      }
    }
  }
);
...
export const selectProbePositions = (state: RootState) => state.multimeter.probePositions;
