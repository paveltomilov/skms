...
import {
	actuatorsConnectionsLeft,
	actuatorsConnectionsRight,
	Connection,
} from '@/shared/configs/actuatorConnections';
		...
		return (
			<>
				{connections.map((connection, index: number) => {
					...
					const status = screwStates[connection.point];
					return (
						<div
							className={`${wrapClass}${
								trigger ? ` ${wrapReverseClass}` : ''
							}`}
							key={`${connection.marker}-${index}`}
						>
							<Provod
								className={`${provodClass}${
									trigger ? ` ${provodReverseClass}` : ''
								}`}
							...
							<ScrewConnection
								...
								className={`${connectionClass}${
									trigger ? ` ${connectionReverseClass}` : ''
								}`}
			...
			<div className={styles.actuator_container_inner}>
				<div className={styles.inner_left}>
					<ActuatorSide
						...
						connectionReverseClass={
							styles.inner_left__reverse__bottom
						}
					/>
				</div>
				<div className={styles.inner_right}>
					<ActuatorSide
						...
						connectionReverseClass={
							styles.inner_right__reverse__bottom
						}
					/>
				</div>
			</div>
