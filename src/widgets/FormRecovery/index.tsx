...
import {
	...
	setNewPassword as apiSetNewPassword,
} from '@/shared/api';
...
const FormRecovery: FC<FormRecoveryProps> = ({ steps, setSteps, isOpen }) => {
	...
	const handleSubmit: FormEventHandler<HTMLFormElement> = async event => {
		...
		if (steps === 2) {
			...
			try {
				const sessionToken =
					localStorage.getItem('recovery:session_token') || '';
				if (!password || !confirmPassword || !sessionToken) return;
				const res = await apiSetNewPassword(
					sessionToken,
					password,
					confirmPassword,
				);
				...
				<>
					...
					<LoginInput
						...
						warn={
							...
							validationStatus.confirm_password === 2
						}
						errorMessage={
							serverErrors.confirm_password
								? configMap.confirm_password?.errorMessage
								: undefined
						}
						warnMessage={
							!serverErrors.confirm_password &&
