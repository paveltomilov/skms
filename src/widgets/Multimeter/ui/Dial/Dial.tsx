...
export const Dial: React.FC<DialProps> = ({ currentMode, modeAngles, onModeSelect }) => {
  ...
  const propsRef = useRef({ currentMode, modeAngles, onModeSelect }); 

  useEffect(() => {
    propsRef.current = { currentMode, modeAngles, onModeSelect };
  }, [currentMode, modeAngles, onModeSelect]);
  ...
  const handleDragMove = useCallback((event: MouseEvent | TouchEvent) => {
    const { currentMode: currentModeFromProp, modeAngles: anglesFromProp, onModeSelect: onModeSelectFromProp } = propsRef.current;
    ...
    const closestMode = findClosestMode(angleDeg, anglesFromProp);
    const snappedAngle = anglesFromProp[closestMode]; 


    setRotation(snappedAngle);

    if (closestMode !== currentModeFromProp) {
       onModeSelectFromProp(closestMode);
    }
